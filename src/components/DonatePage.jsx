...
const UpiIcon = () => <img src="https://upload.wikimedia.org/wikipedia/commons/e/e1/UPI-Logo-vector.svg" alt="UPI" className="h-7"/>;
const VisaIcon = () => <img src="https://upload.wikimedia.org/wikipedia/commons/5/5e/Visa_Inc._logo.svg" alt="Visa" className="h-8"/>;
const MastercardIcon = () => <img src="https://upload.wikimedia.org/wikipedia/commons/2/2a/Mastercard-logo.svg" alt="Mastercard" className="h-9"/>;
const RupayIcon = () => (
  <svg width="80" height="24" viewBox="0 0 105 32" fill="none" xmlns="http://www.w3.org/2000/svg" className="h-9">
    <path d="M19.991 1.773h-4.24l-7.98 21.84v7.92h4.8v-7.92h.72l3.6 7.92h4.08l-4.08-9.3L25.231.773h-4.8l-4.44 11.04L19.991 1.773z" fill="#F79839"/>
    <path d="M10.151 21.093l4.32-10.56 4.32 10.56h-8.64z" fill="#3563A9"/>
    <path d="M37.351 16.533v17.16h4.8v-17.16h6.08v-4.64h-16.96v4.64h6.08z" fill="#F79839"/>
    <path d="M54.631.773h-4.8v33.6h4.8v-33.6z" fill="#F79839"/>
    <path d="M74.551 17.733c0-10.32-5.28-17.52-14.4-17.52s-14.4 7.2-14.4 17.52c0 10.32 5.28 17.52 14.4 17.52s14.4-7.2 14.4-17.52zm-4.8 0c0 7.68-4.24 13.68-9.6 13.68s-9.6-6-9.6-13.68c0-7.68 4.24-13.68 9.6-13.68s9.6 6 9.6 13.68z" fill="#3563A9"/>
    <path d="M88.551.773h-4.8v33.6h14.52v-4.64h-9.72v-28.96z" fill="#3563A9"/>
    <path d="M104.231 17.733c0-10.32-5.28-17.52-14.4-17.52s-14.4 7.2-14.4 17.52c0 10.32 5.28 17.52 14.4 17.52s14.4-7.2 14.4-17.52zm-4.8 0c0 7.68-4.24 13.68-9.6 13.68s-9.6-6-9.6-13.68c0-7.68 4.24-13.68 9.6-13.68s9.6 6 9.6 13.68z" fill="#F79839"/>
  </svg>
);
...
const impactItems = [
  { amount: 500, text: "Plants 5 trees", icon: "🌳" },
  { amount: 1000, text: "Clean water for a family", icon: "💧" },
  { amount: 5000, text: "Funds a school program", icon: "🏫" },
  { amount: 10000, text: "Restores 1 acre of forest", icon: "🌲" },
];
        ...
        <div className="absolute inset-0 overflow-hidden pointer-events-none z-0">
            {Array.from({ length: count }).map((_, i) => {
                const style = {
                    ...
                    '--rotate-end': `${Math.random() > 0.5 ? '' : '-'}${Math.random() * 720}deg`,
                };
                return (
                    <span key={i} className="absolute text-2xl animate-fall" style={style}>
                        {icon}
                    </span>
                );
            })}
    ...
    <div className="min-h-screen  text-gray-800 font-sans p-4 sm:p-6 lg:p-8">
      ...
      <div className="max-w-6xl mx-auto">
            ...
            <div className="mt-auto">
              ...
              <div className="space-y-3">
                {impactItems.map((item) => (
                  ...
                  >
                    <IconRain icon={hoveredIcon === item.icon ? item.icon : null} />
                    <div className="flex items-center z-10">
                      <span className="text-2xl mr-4">{item.icon}</span>
                      <span className="font-semibold">₹{item.amount.toLocaleString("en-IN")}</span>
                    </div>
                    <span className="text-sky-200 text-right z-10">{item.text}</span>
                  </div>
                ))}
              </div>
            </div>
        ...
        <footer className="text-center mt-12 text-gray-500 text-sm">
          <p>&copy; {new Date().getFullYear()} World Green Line. All Rights Reserved.</p>
          <p>A registered non-profit organization. Donations are tax-deductible under Section 80G.</p>
        </footer>
      </div>
    </div>
  );
...
const FormField = ({ 
  ...
  <div>
    ...
    <div className="relative">
      {isCurrency && <span className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-400 font-bold">₹</span>}
      <input
        type={type}
        value={value}
        onChange={onChange}
...
const SuccessOverlay = () => (
  <div className="absolute inset-0 bg-white flex flex-col items-center justify-center text-center p-8 rounded-2xl z-10">
    ...
    <p className="text-gray-600 max-w-sm mb-4">
      Your generous donation is a huge step towards a greener future. A receipt has been sent to your email.
    </p>
...
const ErrorOverlay = ({ onRetry }) => (
  <div className="absolute inset-0 bg-white flex flex-col items-center justify-center text-center p-8 rounded-2xl z-10">
    ...
    <p className="text-gray-600 max-w-sm mb-6">
      We couldn't process your donation. Please check your details and try again.
    </p>
    <button
      onClick={onRetry}
